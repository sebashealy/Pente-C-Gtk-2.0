...
Profesor Tortolero
------------------------------------------------------------------------------------------------------------------
Este modulo se encraga de la jugabilidad del modo jugador contra jugador. Incluye los modulos encargados de:
- Desplegar el tablero
- Desplegar el interfaz del juego
   - Tiempo
   - Fichas
   - Nombres
   - Menu
   - Etc.
- Interpretar cuando el usuario pone una ficha
- Verificar si se cumple un caso de victoria
...
gboolean cerrar_fenetre(GtkWidget *widget, GdkEvent *event, gpointer data);
void informacion(GtkWidget *boton_creditos, gpointer data);
void instrucciones(GtkWidget *boton_reglas, gpointer data);
void color_boton(GtkWidget *widget, gchar player);
extern void pantalla_ganador_jvj(GtkWidget *widget, char letra);
extern void juego(gint row, gint column, gchar matriz[20][20]);
void imprime(gchar matriz[20][20]);
int verifica_5(gchar matriz[20][20], char check);
int verificador_4(gchar matriz[20][20], char check); //funcion que cuenta el numero de lineas rectas echas y condicionamos para verificar si es igual a 4
int verificar_encerradas_hi(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_hd(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_v_arriba(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_v_abajo(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_diagonal_arrd(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_diagonal_arr_iz(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_diagonal_abiz(gchar matriz[20][20], gint row, gint column);
int verificar_encerradas_diagonal_abd(gchar matriz[20][20], gint row, gint column);
...
void button_clicked(GtkWidget *widget, gpointer data)
{
 ...
 GtkWidget *botones[20][20];
	...
	if (*contador_X == *contador_O)
	  {
	    player = 'X';  // Si ambos contadores son iguales, es el turno de 'X'
	  }
	else
	  {
	    if (*contador_X > *contador_O)
	      {
		player = 'O';  // Si hay más 'X' que 'O', entonces es el turno de 'O'
	      }
	    else
	      {
		player = 'X';  // En cualquier otro caso (más 'O' que 'X'), es el turno de 'X'
	      }
	  }
	
        matriz[row][column] = player;
        color_boton(widget, player);
        if (player == 'X') {
            (*contador_X)++;
        } else {
            (*contador_O)++;
        ...
        if (verifica_5(matriz, player)) {
            pantalla_ganador_jvj(NULL, player);
        }

        if (verificador_4(matriz, player) >= 5) {
            pantalla_ganador_jvj(NULL, player);
        }
	...
	if(column >= 3) {
	  ...
	  if(verificar_encerradas_hi(matriz, row, column) > 0)
	    {
	      matriz[row][column - 1] = '.';
	      matriz[row][column - 2] = '.';
	...
	if(column <= 17){
	    ...
	    if(verificar_encerradas_hd(matriz, row, column) > 0){
	      matriz[row][column + 1] = '.';
	      matriz[row][column + 2] = '.';
	    }
	...
	if(column >= 3 && row <= 17){
	   if(verificar_encerradas_diagonal_abiz(matriz, row, column) > 0)
	    {
	...
	if(column >= 3 && row >= 3){
	   if(verificar_encerradas_diagonal_arr_iz(matriz, row, column) > 0)
	    {
	      matriz[row - 1][column - 1] = '.';
	      matriz[row - 2][column - 2] = '.';
	   ...
	   if(verificar_encerradas_diagonal_arrd(matriz, row, column) > 0)
	    {
	      matriz[row - 1][column + 1] = '.';
	      matriz[row - 2][column + 2] = '.';
	    }//verificamos si diagonal abajo hacia la izquierda hacia la izquierda hay fichas atrapadas y si las hay las eliminamos de la matriz	  
	...
	if(column <= 17 && row <= 17){
	   if(verificar_encerradas_diagonal_abd(matriz, row, column) > 0)
	    {
	      matriz[row + 1][column + 1] = '.';
	      matriz[row + 2][column + 2] = '.';
	    }//verificamos si diagonal arriba hacia la izquierda hay fichas atrapadas y si las hay las eliminamos de la matriz	
	...
	if(row >=3){
	   if(verificar_encerradas_v_arriba(matriz, row, column) > 0)
	    {
	      matriz[row - 1][column] = '.';
	      matriz[row - 2][column] = '.';
	    }
	...
	if(row <= 17){
	    if(verificar_encerradas_v_abajo(matriz, row, column) > 0)
	    {
	      matriz[row + 1][column] = '.';
	      matriz[row + 2][column] = '.';
	    }
...
void pantalla_juego(GtkWidget *widget, gpointer data)
{
 ...
 g_object_set_data(G_OBJECT(ventana_juego), "contador_O", contador_O);
  ...
  for(row = 0; row < 20; row++)
      ...
      for(column = 0; column < 20; column++)
	{
	  ...
	  g_signal_connect(botones, "clicked", G_CALLBACK(button_clicked), matriz);
	}
    }
  ...
  g_signal_connect(G_OBJECT(menu_item_instrucciones), "activate", G_CALLBACK(instrucciones), NULL );
  g_signal_connect(G_OBJECT(menu_item_creditos), "activate", G_CALLBACK(informacion), NULL );
  g_signal_connect(G_OBJECT(menu_item_jvj), "activate", G_CALLBACK(cerrar_fenetre), NULL);
  g_signal_connect(G_OBJECT(menu_item_jvj), "activate", G_CALLBACK(pantalla_juego), NULL);
  ...
  g_timeout_add_seconds(1, update_time_label, label);
}
...
int verifica_5(gchar matriz[20][20], char check) //funcion para checar si existe una linea recta de 5 en el programa
{
  int count;
